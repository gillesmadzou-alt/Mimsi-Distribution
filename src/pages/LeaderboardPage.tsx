...
export default function LeaderboardPage({ onNavigate }: { onNavigate?: (page: string) => void }) {
  ...
  const loadStats = useCallback(async () => {
    ...
    if (result.fromCache && result.data && typeof result.data === 'object' && 'batches' in result.data) {
      const cached = result.data as any;
      const start = startISO.slice(0, 10);
      const end = endISO.slice(0, 10);
      if (category === 'drivers') {
        const deposits = new Map<string, number>();
        (cached.deposits ?? []).filter((d: any) => d.deposited_at?.slice(0, 10) >= start && d.deposited_at?.slice(0, 10) <= end).forEach((d: any) => deposits.set(d.batch_id, (deposits.get(d.batch_id) ?? 0) + Number(d.amount_fcfa ?? 0)));
        const rows = new Map<string, DriverRank>();
        (cached.batches ?? []).filter((b: any) => b.batch_date >= start && b.batch_date <= end).forEach((b: any) => {
          if (!rows.has(b.driver_id)) rows.set(b.driver_id, { id: b.driver_id, name: b.driver?.full_name ?? '—', zone: b.driver?.zone ?? '', phone: b.driver?.phone_primary ?? null, total_delivered: 0, total_sold: 0, total_returned: 0, revenue: 0, collected: 0, outstanding: 0, return_rate: 0, rank: 0 });
          const row = rows.get(b.driver_id)!;
          row.total_delivered += Number(b.quantity ?? 0); row.total_returned += Number(b.pots_returned ?? 0); row.revenue += deposits.get(b.id) ?? 0;
        });
        (cached.receivables ?? []).filter((r: any) => r.created_at?.slice(0, 10) >= start && r.created_at?.slice(0, 10) <= end).forEach((r: any) => { const row = rows.get(r.driver_id); if (row) { row.collected += Number(r.amount_paid ?? 0); row.outstanding += Number(r.amount_fcfa ?? 0) - Number(r.amount_paid ?? 0); } });
        const sorted = Array.from(rows.values()).map((row) => ({ ...row, total_sold: row.total_delivered - row.total_returned, return_rate: row.total_delivered ? row.total_returned / row.total_delivered * 100 : 0 })).sort((a, b) => b.total_sold - a.total_sold);
        sorted.forEach((row, index) => { row.rank = index + 1; }); setDriverRows(sorted);
      } else if (category === 'kneaders') {
        const rows = new Map<string, KneaderRank>();
        (cached.doughDeliveries ?? []).filter((d: any) => d.delivery_date >= start && d.delivery_date <= end).forEach((d: any) => { if (!rows.has(d.kneader_id)) rows.set(d.kneader_id, { id: d.kneader_id, name: d.kneader?.full_name ?? '—', phone: d.kneader?.phone ?? null, deliveries_count: 0, total_buckets: 0, total_weight_kg: 0, avg_weight_per_delivery: 0, rank: 0 }); const row = rows.get(d.kneader_id)!; row.deliveries_count++; row.total_buckets += Number(d.bucket_count ?? 0); row.total_weight_kg += Number(d.total_weight_kg ?? 0); });
        const sorted = Array.from(rows.values()).map((row) => ({ ...row, avg_weight_per_delivery: row.deliveries_count ? row.total_weight_kg / row.deliveries_count : 0 })).sort((a, b) => b.total_weight_kg - a.total_weight_kg); sorted.forEach((row, index) => { row.rank = index + 1; }); setKneaderRows(sorted);
      } else {
        const rows = new Map<string, BakerRank>();
        (cached.production ?? []).filter((r: any) => r.production_date >= start && r.production_date <= end).forEach((r: any) => { if (!rows.has(r.baker_id)) rows.set(r.baker_id, { id: r.baker_id, name: r.baker?.full_name ?? '—', phone: r.baker?.phone ?? null, total_pots: 0, pots_burned: 0, madeleines_good: 0, madeleines_burned: 0, madeleines_broken: 0, madeleines_defective: 0, burn_rate: 0, defect_rate: 0, efficiency: 0, rank: 0 }); const row = rows.get(r.baker_id)!; row.total_pots += Number(r.quantity ?? 0); row.pots_burned += Number(r.pots_burned ?? 0); row.madeleines_good += Number(r.madeleines_good ?? 0); row.madeleines_burned += Number(r.madeleines_burned ?? 0); row.madeleines_broken += Number(r.madeleines_broken ?? 0); row.madeleines_defective += Number(r.madeleines_defective ?? 0); });
        const sorted = Array.from(rows.values()).map((row) => ({ ...row, burn_rate: row.total_pots ? row.pots_burned / row.total_pots * 100 : 0, defect_rate: row.madeleines_good ? (row.madeleines_burned + row.madeleines_broken + row.madeleines_defective) / row.madeleines_good * 100 : 0, efficiency: row.total_pots - row.pots_burned })).sort((a, b) => b.efficiency - a.efficiency); sorted.forEach((row, index) => { row.rank = index + 1; }); setBakerRows(sorted);
      }
    }

...
export default function CompliancePage({ onNavigate }: { onNavigate?: (page: string) => void }) {
  ...
  const loadAll = useCallback(async () => {
    ...
    if (result.data) {
      // Older cache entries used a different shape. Guard them so that a stale
      // offline cache can never crash the page while the next precache refreshes it.
      setDiscrepancies(Array.isArray(result.data.discrepancies) ? result.data.discrepancies : []);

  ...
  const buildAccountLedger = (accountType: 'cash' | 'bank') => {
    ...
    const allRows = relevant.map((entry) => {
      ...
      return {
        ...
        accountClass: String(entry.account_class),
        account: `${entry.account_number} — ${entry.account_label}`,
        nature: entry.operation_nature,
        counterpart: `${entry.counterpart_account_number} — ${entry.counterpart_account_label}`,
    ...
    return {
      columns: [
        ...
        { header: 'Classe', key: 'accountClass' },
        { header: 'Compte', key: 'account' },
        { header: 'Nature', key: 'nature' },
        { header: 'Contrepartie', key: 'counterpart' },
  ...
  const buildClientLedger = () => {
    ...
    return {
      ...
      rows: periodRows.map((row) => ({
        date: fmtDate(row.date), reference: row.reference, journal: row.journal, accountClass: row.accountClass,
        account: row.account, nature: row.nature, counterpart: row.counterpart, client: row.client, label: row.label,

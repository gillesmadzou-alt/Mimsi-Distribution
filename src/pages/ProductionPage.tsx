...
export default function ProductionPage({ onNavigate }: { onNavigate?: (page: string) => void }) {
  ...
  const loadAll = useCallback(async () => {
    ...
    if (result.data) {
      ...
      setBakers(allBakers);
      setKneaders(allKneaders);
      setRecords(allRecords);
      setDoughDeliveries(allDough);
      setDoughBatches(allBatches);
      setPotTypes(allPots);
      setPendingBakerReqs(allBakerReqs);
      setPendingKneaderReqs(allKneaderReqs);
      if (profile) {
        const me = allBakers.find((b) => b.profile_id === profile.id);
        setMyBaker(me ?? null);
        const myKnead = allKneaders.find((k) => k.profile_id === profile.id);

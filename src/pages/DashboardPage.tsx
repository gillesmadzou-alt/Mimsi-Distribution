...
export default function DashboardPage({ onNavigate }: { onNavigate?: (page: string) => void }) {
  ...
  useEffect(() => {
    const refreshFromLocalQueue = () => { loadAll(); };
    window.addEventListener('offline-queue-changed', refreshFromLocalQueue);
    return () => window.removeEventListener('offline-queue-changed', refreshFromLocalQueue);
  }, [loadAll]);
        ...
        <div className="flex justify-end">
          ...
          >
            ...
            <span className="rounded-full bg-amber-500 px-2 py-0.5 text-xs text-white">
              {stats.lowStockPots.length + stats.lowEmptyPots.length + stats.lowLids.length}
            </span>
          </button>
        </div>
      )}

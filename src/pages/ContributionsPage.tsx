      ...
      {loading ? <div className="text-center py-20 text-gray-400">Chargement…</div>
        ...
        : <div className="bg-white rounded-2xl shadow-sm border border-gray-100 divide-y divide-gray-50">
          {filteredPoints.map((point) => {
            ...
            const progress = point.quota_amount > 0 ? Math.min(100, (point.quota_paid / point.quota_amount) * 100) : 0;
            return <button key={point.id} onClick={() => setSelectedPoint(point)} className="w-full text-left px-4 sm:px-5 py-4 flex items-center gap-3 hover:bg-gray-50 transition-colors">
              ...
              <div className="text-right shrink-0"><p className="text-sm font-bold text-gray-900">{formatFCFA(remaining)}</p><p className="text-xs text-gray-400">reste sur {formatFCFA(point.quota_amount)}</p></div>

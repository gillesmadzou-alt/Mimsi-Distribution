...
export default function BatchesPage() {
  ...
  const deleteBatchCore = async (id: string) => {
    ...
    const delivered = batch.pots_delivered ?? 0;
  ...
  const confirmCloseBatch = async () => {
    ...
    if (batch) {
      await supabase.from('delivery_events').insert({
        ...
        batch_id: id,
        driver_id: batch.driver_id,
        quantity: batch.pots_delivered,
        description: `Tournée ${batch.batch_code} clôturée — ${batch.pots_delivered}/${batch.quantity ?? 0} pots déposés`,
      });
    }
                ...
                >
                  <div className="flex items-center gap-4">
                    ...
                    <div className="text-right shrink-0">
                      ...
                      <p className="font-bold text-gray-900">{batch.pots_delivered} / {(() => {
                        const bpts = batch.batch_pot_types ?? [];
                      ...
                      <div className="mt-3 flex items-center gap-2">
                        <div className="flex-1 h-2 rounded-full bg-gray-100 overflow-hidden">
                          <div
                            ...
                            style={{ width: `${totalQty > 0 ? (batch.pots_delivered / totalQty) * 100 : 0}%` }}
                          />
                        </div>
                        <span className="text-xs text-gray-500">{Math.max(0, totalQty - batch.pots_delivered)} restants</span>
                      </div>
                    ) : null;
                  })()}
                </div>

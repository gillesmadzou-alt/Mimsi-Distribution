...
import { getAllCachedData } from '@/lib/readCache';
...
export type PageId =
  ...
  | 'observations' | 'expenses' | 'attendance';
...
export default function AppShell({ current, onNavigate, onBack, canGoBack, children }: AppShellProps) {
  const { profile, signOut, offlineMode, manualOffline, setManualOffline } = useAuth();
  ...
  const handleToggleOffline = async () => {
    ...
    setPreCaching(true);
    showProgress('Vérification du cache local…');
    try {
      ...
      if (cacheKeys.length > 0) {
        ...
        finishProgress(`Mode hors-ligne activé — ${cacheKeys.length} pages en cache`);
      } else {
        showProgress('Préchargement des données…');
        await precacheAllData((p) => {
          updateProgress(Math.round((p.done / p.total) * 100));
        });
        updateProgress(100);
        setManualOffline(true);
        finishProgress('Mode hors-ligne activé — données prêtes');
      }
    } catch {
      setProgress(null);
      toast('Erreur lors du préchargement. Réessayez.', 'error');
    }
    setPreCaching(false);
  };
  ...
  const handleBackup = async () => {
    ...
    try {
      ...
      if (payload.errors && payload.errors.length > 0) {
        finishProgress(`Sauvegarde partielle (${payload.errors.length} tables en échec). Cache local mis à jour.`);
      } else {
        finishProgress('Sauvegarde complète. Cache local mis à jour.');
      }
    } catch (err) {
      console.error(err);
      setProgress(null);
      ...
      <nav className="flex-1 px-2 py-3 overflow-y-auto">
        ...
        <div className="space-y-1.5 mt-3 pt-3 border-t border-gray-100">
          ...
          >
            ...
            <span>{preCaching ? 'Préchargement…' : manualOffline ? 'Mode hors-ligne actif' : 'Activer le mode hors-ligne'}</span>
          </button>
          <button
      ...
      <div className="lg:ml-64">
        <header className="bg-white border-b border-gray-200 px-4 sm:px-8 py-4 sticky top-0 z-20 flex items-center justify-between gap-3">
          ...
          <div className="flex items-center gap-2">
            {offlineMode && (
              <span className="flex items-center gap-1.5 px-2.5 py-1.5 rounded-lg bg-amber-50 text-amber-700 text-xs font-medium border border-amber-200">

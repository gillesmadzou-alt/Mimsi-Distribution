...
CREATE TABLE public.marketing_orders (
  ...
  -- Identifiant du message/lead côté plateforme, pour éviter les doublons
  -- si le webhook renvoie deux fois le même évènement.
  external_id text,
  raw_payload jsonb,
  notes text,
...
-- Le rôle Directeur(trice)/Administrateur (access_level >= 4) pilote le
-- marketing. Le webhook automatisé passe par la clé de service (edge
-- function), qui contourne RLS, donc aucune policy anon/public n'est requise.
